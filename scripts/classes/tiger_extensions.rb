class TigerExtension
  ...
  def initialize (jmeter_jmx_path,tiger_extension_path)
    @jmeter_jmx_path=jmeter_jmx_path
    @tiger_extension_path=tiger_extension_path    
  end

  def extend_jmeter_jmx(data_folder_path)
    $logger.info "Extending JMeter scenario #{@jmeter_jmx_path} with #{@tiger_extension_path} content"
    ...
    FileUtils.cp @tiger_extension_path,data_folder_path
